...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export aligned disease-90 hybrid embeddings")
    ...
    fieldnames = ["node_id", "coding", "meaning", "depth", "top_branch_id"] + [f"dim_{index+1}" for index in range(dim)]
    rows = []
    for node_id, vector in zip(objects, embeddings):
        ...
        rows.append(row)
    write_tsv(args.out_tsv, rows, fieldnames)
    write_json(
        args.stats_json,
        {
